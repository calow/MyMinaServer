...
public class PushUnReadMessageHandler implements CIMRequestHandler {
	
	protected final Logger logger = Logger
			.getLogger(PushUnReadMessageHandler.class);
	...
	public ReplyBody process(CIMSession ios, SentBody message) {
		...
		try {
			...
			reply.setMessage(messageService.getUnReadMessageList(account));
		} catch (Exception e) {
			reply.setCode(CIMConstant.ReturnCode.CODE_500);
			e.printStackTrace();
			logger.error("拉取未读消息失败", e);
		}
		return reply;
	}
